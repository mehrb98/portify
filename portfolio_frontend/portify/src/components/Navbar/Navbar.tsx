...
export default function Navbar() {
  ...
  const toggleMenu = () => setUiState((prev) => ({ ...prev, isMenuOpen: !prev.isMenuOpen }));

  return (
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} 
         className={`fixed w-full z-50 ${uiState.isScrolled ? "backdrop-blur-2xl bg-background/90 shadow-2xl shadow-primary/10"
            ...
            <div className="flex items-center justify-between">
               ...
               <div className="hidden md:flex items-center gap-6">
                  <div className="flex items-center gap-6 bg-background/80 px-4 py-2 rounded-full border border-white/5 shadow-lg shadow-primary/5">
                     {navItems.map((item, i) => <MenuItem key={item.name} index={i} href={item.href}>{item.name}</MenuItem>)}
                  </div>

                  <div className="h-6 w-px bg-white/10 mx-2" />

                  {socialLinks.map((social, i) => <SocialLink  key={"social_link_" + i} link={social.link} icon={social.icon} />)}
               </div>
